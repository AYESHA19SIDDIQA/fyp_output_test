...
class Fixation:
    ...
    y: float           # Position (vertical - could be pixel position)
    channels: List[str]  # FIXED: Use correct field name (singular in JSON)
    num_points: int
...
class EEGGazeFixationDataset(Dataset):
    ...
    def __init__(self, data_dir: str, 
        ...
        csv_file = "/kaggle/input/gazedata/results/data/data_processed/results0/data.csv"
    ...
    def _convert_fixations_to_gaze_map(self, 
        ...
        for fixation_idx, fixation in enumerate(fixations):
            ...
            if end_idx > start_idx:
                ...
                # RAW application of weight - preserve absolute values
...
# def debug_specific_sample():
#     """Debug the specific sample 0000037_P0_S2.json"""
    
#     print("=" * 80)
#     print("DEBUGGING: 0000037_P0_S2.json")
#     print("=" * 80)
    
#     # Create dataset with debug mode
#     dataset = EEGGazeFixationDataset(
#         data_dir="results/data/data_processed/results0/train",
#         gaze_json_dir="results/gaze",
#         debug=True,
#         debug_sample="0000037_P0_S2.json",
#         target_length=15000,
...
#     # Find the EEG file that matches our debug sample
#     matching_indices = []
#     for idx, eeg_file in enumerate(dataset.eeg_files):
#         eeg_basename = os.path.basename(eeg_file)
#         # Look for files that match 0000037 and S2
#         if "0000037" in eeg_basename and "_S2" in eeg_basename:
#             matching_indices.append(idx)
#             print(f"\nFound matching EEG file at index {idx}:")
#             print(f"  {eeg_basename}")
            
#             # Check what gaze file it maps to
#             gaze_file = dataset.eeg_to_gaze.get(eeg_file)
#             if gaze_file:
#                 print(f"  Maps to gaze file: {os.path.basename(gaze_file)}")
#             else:
#                 print(f"  No gaze file mapped!")
    
#     if not matching_indices:
#         print("\n✗ No matching EEG files found!")
#         # List some EEG files to see what exists
#         print("\nFirst 10 EEG files:")
#         for i, eeg_file in enumerate(dataset.eeg_files[:40]):
#             print(f"  {i}: {os.path.basename(eeg_file)}")
#         return
    
#     # Process each matching sample
#     for idx in matching_indices[:2]:  # Process first 2 matches
#         print(f"\n" + "="*80)
#         print(f"PROCESSING SAMPLE {idx}")
#         print("="*80)
        
#         try:
#             # This will trigger __getitem__ with all debug output
#             sample = dataset[idx]
            
#             print(f"\n✓ Successfully loaded sample {idx}")
#             print(f"  EEG shape: {sample['eeg'].shape}")
#             print(f"  Gaze shape: {sample['gaze'].shape}")
#             print(f"  Label: {sample['label']}")
#             print(f"  Num fixations: {sample['num_fixations']}")
            
#             # Analyze gaze map
#             gaze = sample['gaze'].numpy()
#             print(f"\nGAZE MAP ANALYSIS:")
#             print(f"  Shape: {gaze.shape} (channels × time)")
#             print(f"  Min: {gaze.min():.8f}")
#             print(f"  Max: {gaze.max():.8f}")
#             print(f"  Mean: {gaze.mean():.8f}")
#             print(f"  Std: {gaze.std():.8f}")
            
#             # Check if all values are the same
#             unique_vals = np.unique(gaze.round(8))
#             print(f"  Unique values (rounded to 8 decimals): {len(unique_vals)}")
#             if len(unique_vals) <= 5:
#                 print(f"  Values: {unique_vals}")
            
#             # Temporal analysis
#             temporal_mean = gaze.mean(axis=0)
#             print(f"\nTEMPORAL ANALYSIS (across channels):")
#             print(f"  Temporal mean min: {temporal_mean.min():.6f}")
#             print(f"  Temporal mean max: {temporal_mean.max():.6f}")
#             print(f"  Temporal mean std: {temporal_mean.std():.6f}")
            
#             # Find peaks in time dimension
#             threshold = temporal_mean.mean() + temporal_mean.std()
#             peaks = np.sum(temporal_mean > threshold)
#             print(f"  Time points > mean+std: {peaks}/{len(temporal_mean)}")
            
#             # Channel analysis
#             channel_mean = gaze.mean(axis=1)
#             print(f"\nCHANNEL ANALYSIS (across time):")
#             print(f"  Channel with max attention: {np.argmax(channel_mean)}")
#             print(f"  Channel with min attention: {np.argmin(channel_mean)}")
            
#             # Check for any non-zero values
#             non_zero = np.sum(gaze > 0.1)
#             total_elements = gaze.size
#             print(f"\nATTENTION DISTRIBUTION:")
#             print(f"  Elements > 0.1: {non_zero}/{total_elements} ({(non_zero/total_elements)*100:.1f}%)")
#             print(f"  Elements > 0.5: {np.sum(gaze > 0.5)}/{total_elements}")
#             print(f"  Elements > 0.9: {np.sum(gaze > 0.9)}/{total_elements}")
...
#     """Directly check the JSON file content"""
#     import json
    
#     json_path = "results/gaze/0000037_P0_S2.json"
#     print(f"\n" + "="*80)
#     print(f"DIRECT JSON ANALYSIS: {json_path}")
#     print("="*80)
...
#         if 'fixations' in data:
#             fixations = data['fixations']
#             print(f"Number of fixations: {len(fixations)}")
            
#             print(f"\nFirst 5 fixations:")
#             for i, fix in enumerate(fixations[:5]):
#                 print(f"  Fixation {i}:")
#                 print(f"    x: {fix.get('x', 'N/A')}")
#                 print(f"    duration: {fix.get('duration', 'N/A')}")
#                 print(f"    channels: {fix.get('channel', fix.get('channels', 'N/A'))}")
#                 print(f"    num_points: {fix.get('num_points', 'N/A')}")
            
#             # Calculate statistics
#             x_values = [f.get('x', 0) for f in fixations if 'x' in f]
#             if x_values:
#                 print(f"\nX-value statistics:")
#                 print(f"  Min: {min(x_values):.2f}s")
#                 print(f"  Max: {max(x_values):.2f}s")
#                 print(f"  Mean: {np.mean(x_values):.2f}s")
#                 print(f"  Range: {max(x_values) - min(x_values):.2f}s")
                
#                 # Check segment
#                 print(f"\nSegment analysis:")
#                 print(f"  Values in 0-300s: {sum(0 <= x <= 300 for x in x_values)}")
#                 print(f"  Values in 300-600s: {sum(300 < x <= 600 for x in x_values)}")
#                 print(f"  Values in 600-900s: {sum(600 < x <= 900 for x in x_values)}")
                
#         else:
#             print("No 'fixations' key found in JSON")
...
#     # Then debug the dataset processing
#     debug_specific_sample()
